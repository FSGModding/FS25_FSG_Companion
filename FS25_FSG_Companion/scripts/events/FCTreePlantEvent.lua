...
function FCTreePlantEvent.emptyNew()
	local self = Event.new(FCTreePlantEvent_mt)

	return self
end

-- Lines 13-26
function FCTreePlantEvent.new(treeType, superFunc, x, y, z, rx, ry, rz, growthState, splitShapeFileId, isGrowing, price, farmId)
...
-- Lines 28-59
function FCTreePlantEvent:readStream(streamId, connection)
...
-- Lines 61-82
function FCTreePlantEvent:writeStream(streamId, connection)
...
-- Lines 84-86
function FCTreePlantEvent:run(connection)
	print("Error: FCTreePlantEvent is not allowed to be executed on a local client")
end
